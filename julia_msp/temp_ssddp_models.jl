...
function asset_management_alm_save(data:: Temp.GoalsData3)
    ...
    model = SDDP.PolicyGraph(
        ...
        @variable(subproblem, stocks >= 0, SDDP.State, initial_value = 0.0)
        @variable(subproblem, bonds >= 0, SDDP.State, initial_value = 0.0)
        @variable(subproblem, cpi >= 0, SDDP.State, initial_value = 0.0)
        ...
        elseif 1 < stage 
            @constraint(
                subproblem,
                (r_stock[markov_state] + inflation) * stocks.in + stocks_buy - stocks_sell== stocks.out)

            @constraint(
                subproblem,
                r_bonds[1] * bonds.in + bonds_buy - bonds_sell== bonds.out)
                
            @constraint(subproblem, cpi.in == cpi.out)
